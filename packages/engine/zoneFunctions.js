...
export const OutOfPlayBlueClick = (selected, setblueCoin, setBlueState, isBlueTurn, BlueState, blueCoin) => {
...
export const OutOfPlayRedClick = (selected1, RedState, setRedState, isRedTurn, redCoin, setredCoin) => {
    const redFilter = RedState.selected.filter((item) => item.id !== selected1.id)
    setRedState({ ...RedState, selected: redFilter })
    isRedTurn.RedOutOfPlay.push(selected1)
    setredCoin(redCoin - 1)
}

export const handleBlueClick = (playerBlue, blueDeck, BlueState, setBlueState) => {
    if (playerBlue) {
        var card = blueDeck
...
const getList = (id, BlueState) => BlueState[id1List[id]]
...
export const onDragBlue = (result, playerBlue, BlueState, setBlueState) => {
    if (playerBlue) {
        ...
        if (source.droppableId === destination.droppableId) {
            const blueOrder = reorder(getList(source.droppableId), source.index, destination.index)
